<?php
    ...
    $id = $_SESSION['uid'];
    $type = $_SESSION['type'];
    $query = "SELECT * FROM login WHERE reg_id = '$id' AND type = 'HOSPITAL'";
    ...
    if ($result && mysqli_num_rows($result) > 0) {
        ...
        echo "<script>alert('HOSPITAL details not found!');</script>";
        exit();
    }
    if (isset($_POST['submit'])) {
            ...
            if ($newpass == $confirmpass) {
                $updateqry = "UPDATE login SET email = '$newemail', password = '$newpass' WHERE reg_id = '$id' AND type = 'HOSPITAL'";
